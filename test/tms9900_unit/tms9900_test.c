/*
 * TMS9900 CPU Core Unit Tests — RP2040 (Pico)
 *
...
/* Run the same test on both cores and report both */
#define FOR_BOTH(setup_fn, prog, len, check_fn) do { \
  setup_fn; load_prog(prog, len); \
  printf("  [ASM] "); run_asm(); check_fn; \
  setup_fn; load_prog(prog, len); \
  printf("  [C]   "); run_c();   check_fn; \
} while(0)
